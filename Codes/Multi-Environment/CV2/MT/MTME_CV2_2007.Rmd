---
title: "Multi-Trait Multi-Environment Analysis - 2007 CV2"
output: pdf_document
...
tst <- which(MT_final$Env == "2007_CAMPO_MOURAO")
ytest <- MT_final[tst,]
ytest2 <- ytest
ytest2$BLUES <- NA
# separating the sets from training population
ytrn <- MT_final[-tst,]
y_final <- rbind(ytest2,ytrn)


  model.MT.ME.CV2.2007 =MCMCglmm(BLUES ~ -1 + trait_env,
  ...
  a_gy = colMeans(model.MT.ME.CV2.2007$Sol)
  ...
  tst_gy = grep("2007_CAMPO_MOURAO",predicted_and_observed_gy$env_trait_hybrid)
  final_gy = predicted_and_observed_gy[tst_gy,]
  acc_mcmcglmm_GY <- cor(final_gy[,2],final_gy[,4])

  a_ne = colMeans(model.MT.ME.CV2.2007$Sol)
  ...
  tst_ne = grep("2007_CAMPO_MOURAO",predicted_and_observed_ne$env_trait_hybrid)
  final_ne= predicted_and_observed_ne[tst_ne,]
  acc_mcmcglmm_NE <- cor(final_ne[,2],final_ne[,4])

  a_gm = colMeans(model.MT.ME.CV2.2007$Sol)
  ...
  tst_gm = grep("2007_CAMPO_MOURAO",predicted_and_observed_gm$env_trait_hybrid)
  final_gm= predicted_and_observed_gm[tst_gm,]
  acc_mcmcglmm_GM <- cor(final_gm[,2],final_gm[,4])


}

save.image("MTME_CV2_2007.RData")  
```
