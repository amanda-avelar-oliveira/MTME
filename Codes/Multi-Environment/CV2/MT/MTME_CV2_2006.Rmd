---
title: "Multi-Trait Multi-Environment Analysis - 2006 CV2"
output: pdf_document
...
tst <- which(MT_final$Env == "2006_CAMPO_MOURAO")
ytest <- MT_final[tst,]
ytest2 <- ytest
ytest2$BLUES <- NA
...
y_final <- rbind(ytest2,ytrn)
  ...
  model.MT.ME.CV2.2006 =MCMCglmm(BLUES ~ -1 + trait_env,
  ...
  a_gy = colMeans(model.MT.ME.CV2.2006$Sol)
  ...
  tst_gy = grep("2006_CAMPO_MOURAO",predicted_and_observed_gy$env_trait_hybrid)
  final_gy = predicted_and_observed_gy[tst_gy,]
  acc_mcmcglmm_GY <- cor(final_gy[,2],final_gy[,4])

  a_ne = colMeans(model.MT.ME.CV2.2006$Sol)
  ...
  tst_ne = grep("2006_CAMPO_MOURAO",predicted_and_observed_ne$env_trait_hybrid)
  final_ne= predicted_and_observed_ne[tst_ne,]
  acc_mcmcglmm_NE <- cor(final_ne[,2],final_ne[,4])

  a_gm = colMeans(model.MT.ME.CV2.2006$Sol)
  ...
  tst_gm = grep("2006_CAMPO_MOURAO",predicted_and_observed_gm$env_trait_hybrid)
...
save.image("MTME_CV2_2006.RData")  
```
