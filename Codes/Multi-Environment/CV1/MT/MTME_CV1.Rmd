---
title: "Multi-Trait Multi-Environment Analysis - CV1"
output: pdf_document
...
for(fold in n_folds){
  ...
  names_hybrids_tst <- MT_final[tst,1, drop= FALSE]
  names_hybrids_tst_unique <- unique(names_hybrids_tst)
  yteste <- MT_final[which(MT_final$H %in% names_hybrids_tst_unique[,1]),]
  # separating the sets from training population
  '%!in%' <- function(x,y)!('%in%'(x,y))
  ytrn <- MT_final[which(MT_final$H %!in% names_hybrids_tst_unique[,1]),]
  dim(ytrn) 


  model.MT.ME.CV1 =MCMCglmm(BLUES ~ -1 + trait_env,
  ...
  a_gy = colMeans(model.MT.ME.CV1$Sol)
  ...
  a_ne = colMeans(model.MT.ME.CV1$Sol)
  ...
  a_gm = colMeans(model.MT.ME.CV1$Sol)
