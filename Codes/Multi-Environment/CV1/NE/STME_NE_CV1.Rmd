---
title: "Single-Trait Multi-Environment Analysis - NE CV1"
output: pdf_document
...
for(fold in n_folds){
  ...
  names_hybrids_tst <- BLUEs_NE[tst,1, drop= FALSE]
  names_hybrids_tst_unique <- unique(names_hybrids_tst)
  yteste <- BLUEs_NE[which(BLUEs_NE$H %in% names_hybrids_tst_unique[,1]),]
  # separating the sets from training population
  '%!in%' <- function(x,y)!('%in%'(x,y))
  ytrn <- BLUEs_NE[which(BLUEs_NE$H %!in% names_hybrids_tst_unique[,1]),]
  dim(ytrn) 
 
  print(paste('Model STME.NE.CV1', fold))

  STME.NE.CV1 = MCMCglmm(NE_BLUE ~ -1 + Env,
            random= ~us(Env):H,
  ...
  blups = colMeans(STME.NE.CV1$Sol)
