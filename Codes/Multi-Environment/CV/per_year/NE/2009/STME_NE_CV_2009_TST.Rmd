---
title: "Single-Trait Multi-Environment (STME) analysis using the same training/testing partition of Single-Environment (SE) analysis - NE 2009"
output: pdf_document
...
```{r eval=FALSE, message=FALSE}
BLUEs_tst <- BLUEs_NE[BLUEs_NE$Env == "2009_CAMPO_MOURAO",]
```
...
for(fold in n_folds){
  ...
  print(paste('Model STME.NE_CV_2009_TST', fold))
  
  STME.NE_CV_2009_TST =  MCMCglmm(NE_BLUE ~ -1 + Env,
                    random= ~us(Env):H,
  ...
  blups = colMeans(STME.NE_CV_2009_TST$Sol)
...
save.image("STME_NE_CV_2009_TST.RData")  
```
