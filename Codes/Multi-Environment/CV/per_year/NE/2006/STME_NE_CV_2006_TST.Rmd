---
title: "Single-Trait Multi-Environment (STME) analysis using the same training/testing partition of Single-Environment (SE) analysis - NE 2006"
output: pdf_document
...
Select the year 2006 as testing set

```{r eval=FALSE, message=FALSE}
BLUEs_tst <- BLUEs_NE[BLUEs_NE$Env == "2006_CAMPO_MOURAO",]
```
...
Adjust the predictive model using a 5 folds cross-validation scheme but using year 2006 as testing set
...
for(fold in n_folds){
  ...
  posi_test <- match(names_test, rownames(BLUEs_NE))
 
    
  print(paste('Model STME.NE_CV_2006_TST', fold))
  
  STME.NE_CV_2006_TST =  MCMCglmm(NE_BLUE ~ -1 + Env,
                    random= ~us(Env):H,
                    ginverse=list(H=Hinv),
                    rcov = ~ us(Env):units,
                    data = BLUEs_NE[-posi_test,], 
                    prior=prior.NonIformative.me,
                    pr=TRUE,
                    nitt=nitt, thin=thin, burnin=burnin)
  
  
  blups = colMeans(STME.NE_CV_2006_TST$Sol)
...
save.image("STME_NE_CV_2006_TST.RData")  
```
