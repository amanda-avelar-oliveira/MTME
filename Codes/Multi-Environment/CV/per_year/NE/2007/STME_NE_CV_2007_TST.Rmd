---
title: "Single-Trait Multi-Environment (STME) analysis using the same training/testing partition of Single-Environment (SE) analysis - NE 2007"
output: pdf_document
...
Select the year 2007 as testing set

```{r eval=FALSE, message=FALSE}
BLUEs_tst <- BLUEs_NE[BLUEs_NE$Env == "2007_CAMPO_MOURAO",]
```
...
Adjust the predictive model using a 5 folds cross-validation scheme but using year 2007 as testing set
...
for(fold in n_folds){
  ...
  print(paste('Model STME.NE_CV_2007_TST', fold))
  
  STME.NE_CV_2007_TST =  MCMCglmm(NE_BLUE ~ -1 + Env,
                    random= ~us(Env):H,
  ...
  blups = colMeans(STME.NE_CV_2007_TST$Sol)
...
save.image("STME_NE_CV_2007_TST.RData")  
```
