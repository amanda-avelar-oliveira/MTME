---
title: "Single-Trait Multi-Environment (STME) analysis using the same training/testing partition of Single-Environment (SE) analysis - GM 2006"
output: pdf_document
...
```{r eval=FALSE, message=FALSE}
BLUEs_tst <- BLUEs_GM[BLUEs_GM$Env == "2006_CAMPO_MOURAO",]
```
...
for(fold in n_folds){
  ...
  print(paste('Model STME.GM_CV_2006_TST', fold))
  
  STME.GM_CV_2006_TST =  MCMCglmm(GM_BLUE ~ -1 + Env,
                    random= ~us(Env):H,
  ...
  blups = colMeans(STME.GM_CV_2006_TST$Sol)
...
save.image("STME_GM_CV_2006_TST.RData")  
```
