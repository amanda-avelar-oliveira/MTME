---
title: "Multi-Trait Multi-Environment (MTME) analysis using the same training/testing partition of Single-Environment (SE) analysis - 2007"
output: pdf_document
...
```{r eval=FALSE, message=FALSE}
MT_tst <- MT_final[MT_final$env == "2007_CAMPO_MOURAO",]
```
...
for(fold in n_folds){
  ...
  posi_teste <- match(yteste$env_trait_hybrid, MT_final$env_trait_hybrid)
    ...
    MTME_CV_2007_TST =MCMCglmm(BLUES ~ -1 + trait_env,
    ...
    a_gy = colMeans(MTME_CV_2007_TST$Sol)
    ...
    a_ne = colMeans(MTME_CV_2007_TST$Sol)
    ...
    a_gm = colMeans(MTME_CV_2007_TST$Sol)
...
save.image("MTME_CV_2007_TST.RData")  
```
