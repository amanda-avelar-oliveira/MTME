---
title: "Single-Trait Multi-Environment (STME) analysis using the same training/testing partition of Single-Environment (SE) analysis - GY 2006"
output: pdf_document
...
```{r eval=FALSE, message=FALSE}
BLUEs_tst <- BLUEs_GY[BLUEs_GY$Env == "2006_CAMPO_MOURAO",]
```
...
for(fold in n_folds){
  ...
  print(paste('Model STME.GY_CV_2006_TST', fold))
  
  STME.GY_CV_2006_TST =  MCMCglmm(GY_BLUE ~ -1 + Env,
                    random= ~us(Env):H,
  ...
  blups = colMeans(STME.GY_CV_2006_TST$Sol)
...
save.image("STME_GY_CV_2006_TST.RData")  
```
