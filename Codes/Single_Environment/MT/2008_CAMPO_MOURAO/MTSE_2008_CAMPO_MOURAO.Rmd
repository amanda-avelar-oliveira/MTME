...
Select the BLUEs from environment 2008_CAMPO_MOURAO
...
for(fold in n_folds){
  ...
  print(paste('Model MTSE.2008_CM', fold))

  MTSE.2008_CM = MCMCglmm(cbind(GY_BLUE, GM_BLUE, NE_BLUE) ~ trait -1 ,
                 random= ~ us(trait):H,
...
blups[[fold]] = colMeans(MTSE.2008_CM$Sol)
ind_test[[fold]] = ytest$H

# Extracting the accuracy from GY
a_gy = colMeans(MTSE.2008_CM$Sol)
...
a_ne = colMeans(MTSE.2008_CM$Sol)
...
a_gm = colMeans(MTSE.2008_CM$Sol)
...
save.image("2008_CAMPO_MOURAO_MT_CV.RData")
```
