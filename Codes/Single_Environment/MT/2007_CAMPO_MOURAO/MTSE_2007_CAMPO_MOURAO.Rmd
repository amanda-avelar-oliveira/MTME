...
Select the BLUEs from environment 2007_CAMPO_MOURAO

```{r eval=FALSE, message=FALSE}
library("dplyr")

BLUEs_selected <- filter(BLUEs, Env == "2007_CAMPO_MOURAO")
```
...
Select in the H matrix only the hybrids from environment 2007_CAMPO_MOURAO

```{r eval=FALSE, message=FALSE}
idx = as.character(unique(MT_scale$H))
Hmatrix = AHAT.inv[idx,idx]
dim(Hmatrix) # 100 x 100
```
...
for(fold in n_folds){
  ...
  print(paste('Model MTSE.2007_CM', fold))

  MTSE.2007_CM = MCMCglmm(cbind(GY_BLUE, GM_BLUE, NE_BLUE) ~ trait -1 ,
                 random= ~ us(trait):H,
...
blups[[fold]] = colMeans(MTSE.2007_CM$Sol)
ind_test[[fold]] = ytest$H

# Extracting the accuracy from GY
a_gy = colMeans(MTSE.2007_CM$Sol)
...
a_ne = colMeans(MTSE.2007_CM$Sol)
...
a_gm = colMeans(MTSE.2007_CM$Sol)
...
save.image("2007_CAMPO_MOURAO_MT_CV.RData")
```
